<?php
$alert_message = ''; // Variable for storing alert messages
...
if (isset($_GET['token'])) {
    ...
    if ($result->num_rows > 0) {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            // Update password in the database and clear reset_token
            $stmt = $conn->prepare("UPDATE users SET password = ?, reset_token = NULL WHERE reset_token = ?");
            $stmt->bind_param("ss", $hashed_password, $token);
            $stmt->execute();

            $alert_message = '<div class="alert alert-success" role="alert">Password has been reset successfully!</div>';
            // Set flag to show the "Back to Login" button
            $show_back_to_login = true; // Flag for showing the button
        }
    } else {
        $alert_message = '<div class="alert alert-danger" role="alert">Invalid token!</div>';
        $show_back_to_login = false; // Keep the "Back to Login" button hidden
    }
} else {
    // If there's no token, set flag to hide the "Back to Login" button
    $show_back_to_login = false;
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Reset Password| Shortener Link</title>
...
<body>
    ...
    <!-- / Content -->
    <script>
        // Hide "Back to Login" button initially
        document.getElementById("back-to-login").style.display = "none";

        // Show "Back to Login" button after successful reset
        <?php if ($show_back_to_login): ?>
