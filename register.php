...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($result->num_rows > 0) {
        ...
        $alert_message = '<div class="alert alert-danger" role="alert">Username already exists. Please choose another one.</div>';
    } else {
        ...
        if ($result->num_rows > 0) {
            ...
            $alert_message = '<div class="alert alert-danger" role="alert">Email is already registered. Please use a different email.</div>';
        } else {
            // If username and email are unique, insert the new user
            $hashed_password = password_hash($password, PASSWORD_DEFAULT);

            // Prepare INSERT query
            $stmt = $conn->prepare("INSERT INTO users (username, password, email, nama) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssss", $uname, $hashed_password, $email, $nama);

            // Execute the query and check if it's successful
            if ($stmt->execute()) {
                $alert_message = '<div class="alert alert-success" role="alert">Account created successfully!</div>';
            } else {
                $alert_message = '<div class="alert alert-danger" role="alert">Error: Could not create account! Error: ' . $stmt->error . '</div>';
            }
        }
    }
}
...
<head>
    ...
    <title>Register | Shortener Link</title>
...
<body>
    ...
    <!-- Page JS -->
    <script src="assets_sneat/js/ui-modals.js"></script>
